...
function _setup(alg) {
  ...
  cfg.generateKey = async (opts) => {
    opts = cfg.configure("generateKey", opts);
    let { algorithm, extractable = true, usages = modes } = opts;
  ...
  cfg.encrypt = async (opts) => {
    let { algorithm, key, iv, adata, plaintext } = cfg.configure("encrypt", opts);
  ...
  cfg.decrypt = async (opts) => {
    let { algorithm, key, iv, adata, ciphertext, tag } = cfg.configure("decrypt", opts);
    algorithm =  prepareAlgorithm(algorithm, iv, adata);
    ...
    opts.plaintext = new Uint8Array(plaintext);
